  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < importedData.length; i += BATCH_SIZE) {
        ...
        await userService.importBulkUsers(batch);
      }
      
      // ✅ Capture IDs/Emails of newly added users for Badge
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                             ...
                             {/* ✅ THE NEW BADGE IS HERE */}
                             {newlyAddedEmails.includes(user.email) && (
                              <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-600 text-white animate-pulse shadow-sm">
                                NEW
                              </span>
                             )}
                          </div>
                      ...
                      <div className="relative">
                        ...
                        {activeDropdown === user.id && (
                          <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-1 z-50">
                            ...
                            <button
                              onClick={() => { openEditModal(user); setActiveDropdown(null); }}
                              className="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                            >
                              ...
                              <div className="border-t border-gray-100 mt-1 pt-1">
                                ...
                                {['student', 'teacher', 'admin'].filter(r => r !== user.role).map(role => (
                                  <button
                                    key={role}
                                    onClick={() => {
                                      setConfirmAction({ user, type: 'role-change', newRole: role });
                            ...
                            <div className="border-t border-gray-100 mt-1 pt-1">
                              <button
                                ...
                                className={`w-full px-4 py-2 text-left text-sm flex items-center gap-2 ${user.status === 'active'
      ...
      <Modal isOpen={!!editUser} onClose={() => { setEditUser(null); resetForm(); }} title="Edit User" size="md">
        <form onSubmit={handleEditUser} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button type="button" variant="secondary" onClick={() => { setEditUser(null); resetForm(); }}>
              Cancel
            </Button>
          </div>
        </form>
